...
function kube_prometheus_stack_set_remote_write_timeout_to_5s() {
  ...
  if [[ -z "$(yq r - 'remoteWrite.[*].remoteTimeout' <<< "${prometheus_spec}")" ]]; then
    # No remoteTimeout to migrate
    return
  fi
  ...
  yq w -i "${TEMP_FILE}" "kube-prometheus-stack.prometheus.prometheusSpec" \
    --from <(echo "${new_remote_write}")
}
